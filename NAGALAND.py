...
#Data Load
NAGALAND=pd.read_csv("D:/labour_project/NAGALAND.csv",encoding="cp1252")
#Title
...
oly_total=NAGALAND[NAGALAND['NIC Name']=='Total']
...
with col1:
    st.write("STATE - NAGALAND")
with col2:
    st.write("STATE CODE : 13")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(NAGALAND)
...
coords = pd.read_csv("D:/labour_project/nagalars.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="NAGALAND District-wise Total Workers Distribution")
