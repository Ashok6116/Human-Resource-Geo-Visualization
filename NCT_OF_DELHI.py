...
#Data Load
NCT_OF_DELHI=pd.read_csv("D:/labour_project/NCT_OF_DELHI.csv",encoding="cp1252")
#Title
...
oly_total=NCT_OF_DELHI[NCT_OF_DELHI['NIC Name']=='Total']
...
with col1:
    st.write("STATE - NCT_OF_DELHI")
with col2:
    st.write("STATE CODE : 7")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(NCT_OF_DELHI)
...
sunburst_df = NCT_OF_DELHI[
    (NCT_OF_DELHI["Division"] != 0) &
    (NCT_OF_DELHI["Group"] != 0) &
    (NCT_OF_DELHI["Class"] != 0)]
...
coords = pd.read_csv("D:/labour_project/delhi.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="NCT_OF_DELHI District-wise Total Workers Distribution")
