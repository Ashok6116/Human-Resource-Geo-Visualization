...
#Data Load
MAHARASHTRA=pd.read_csv("D:/labour_project/MAHARASHTRA.csv",encoding="cp1252")
#Title
...
oly_total=MAHARASHTRA[MAHARASHTRA['NIC Name']=='Total']
...
with col1:
    st.write("STATE - MAHARASHTRA")
with col2:
    st.write("STATE CODE : 27")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(MAHARASHTRA)
...
sunburst_df = MAHARASHTRA[
    (MAHARASHTRA["Division"] != 0) &
    (MAHARASHTRA["Group"] != 0) &
    (MAHARASHTRA["Class"] != 0)]
...
st.write(new_oly)
coords = pd.read_csv("D:/labour_project/maharatras.csv")
st.write(coords)
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="MAHARASHTRA District-wise Total Workers Distribution")
