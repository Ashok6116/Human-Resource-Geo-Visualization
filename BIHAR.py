...
#Data Load
BIHAR=pd.read_csv("D:/labour_project/BIHAR.csv",encoding="cp1252")
#Title
...
oly_total=BIHAR[BIHAR['NIC Name']=='Total']
...
with col1:
    st.write("STATE - BIHAR")
with col2:
    st.write("STATE CODE : 18")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(BIHAR)
...
'''for i in new_oly['India/States']:
    st.write(i)'''
st.write(new_oly)
coords = pd.read_csv("D:/labour_project/bihars.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="Bihar District-wise Total Workers Distribution")
