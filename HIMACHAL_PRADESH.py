...
#Data Load
HIMACHAL_PRADESH=pd.read_csv("D:/labour_project/HIMACHAL_PRADESH.csv",encoding="cp1252")
#Title
...
oly_total=HIMACHAL_PRADESH[HIMACHAL_PRADESH['NIC Name']=='Total']
...
with col1:
    st.write("STATE - HIMACHAL_PRADESH")
with col2:
    st.write("STATE CODE : 24")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(HIMACHAL_PRADESH)
...
sunburst_df = HIMACHAL_PRADESH[
    (HIMACHAL_PRADESH["Division"] != 0) &
    (HIMACHAL_PRADESH["Group"] != 0) &
    (HIMACHAL_PRADESH["Class"] != 0)]
...
'''for i in new_oly['India/States']:
    st.write(i)'''
st.write(new_oly)
coords = pd.read_csv("D:/labour_project/himachal.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="Gujarat District-wise Total Workers Distribution")
