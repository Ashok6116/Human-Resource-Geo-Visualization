...
#Data Load
UTTAR_PRADESH=pd.read_csv("D:/labour_project/UTTAR_PRADESH.csv",encoding="cp1252")
#Title
...
oly_total=UTTAR_PRADESH[UTTAR_PRADESH['NIC Name']=='Total']
...
with col1:
    st.write("STATE - UTTAR_PRADESH")
with col2:
    st.write("STATE CODE : 9")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(UTTAR_PRADESH)
...
sunburst_df = UTTAR_PRADESH[
    (UTTAR_PRADESH["Division"] != 0) &
    (UTTAR_PRADESH["Group"] != 0) &
    (UTTAR_PRADESH["Class"] != 0)]
...
coords = pd.read_csv("D:/labour_project/uttar.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="UTTAR_PRADESH District-wise Total Workers Distribution")
