...
#Data Load
TAMIL_NADU=pd.read_csv("D:/labour_project/TAMIL_NADU.csv",encoding="cp1252")
#Title
...
oly_total=TAMIL_NADU[TAMIL_NADU['NIC Name']=='Total']
...
with col1:
    st.write("STATE - TAMIL_NADU")
with col2:
    st.write("STATE CODE : 33")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(TAMIL_NADU)
...
sunburst_df = TAMIL_NADU[
    (TAMIL_NADU["Division"] != 0) &
    (TAMIL_NADU["Group"] != 0) &
    (TAMIL_NADU["Class"] != 0)]
...
coords = pd.read_csv("D:/labour_project/tamil.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="TAMIL_NADU District-wise Total Workers Distribution")
