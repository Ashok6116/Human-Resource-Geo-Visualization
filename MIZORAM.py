...
#Data Load
MIZORAM=pd.read_csv("D:/labour_project/MIZORAM.csv",encoding="cp1252")
#Title
...
oly_total=MIZORAM[MIZORAM['NIC Name']=='Total']
...
with col1:
    st.write("STATE - MIZORAM")
with col2:
    st.write("STATE CODE : 15")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(MIZORAM)
...
coords = pd.read_csv("D:/labour_project/mizora.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="MIZORAM District-wise Total Workers Distribution")
