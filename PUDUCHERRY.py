...
#Data Load
PUDUCHERRY=pd.read_csv("D:/labour_project/PUDUCHERRY.csv",encoding="cp1252")
#Title
...
oly_total=PUDUCHERRY[PUDUCHERRY['NIC Name']=='Total']
...
with col1:
    st.write("STATE - PUDUCHERRY")
with col2:
    st.write("STATE CODE : 34")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(PUDUCHERRY)
...
sunburst_df = PUDUCHERRY[
    (PUDUCHERRY["Division"] != 0) &
    (PUDUCHERRY["Group"] != 0) &
    (PUDUCHERRY["Class"] != 0)]
...
coords = pd.read_csv("D:/labour_project/pundicherys.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="PUDUCHERRY District-wise Total Workers Distribution")
