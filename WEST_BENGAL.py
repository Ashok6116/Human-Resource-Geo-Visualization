...
#Data Load
WEST_BENGAL=pd.read_csv("D:/labour_project/WEST_BENGAL.csv",encoding="cp1252")
#Title
...
oly_total=WEST_BENGAL[WEST_BENGAL['NIC Name']=='Total']
...
with col1:
    st.write("STATE - WEST_BENGAL")
with col2:
    st.write("STATE CODE : 19")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(WEST_BENGAL)
...
sunburst_df = WEST_BENGAL[
    (WEST_BENGAL["Division"] != 0) &
    (WEST_BENGAL["Group"] != 0) &
    (WEST_BENGAL["Class"] != 0)]
...
coords = pd.read_csv("D:/labour_project/west.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="WEST_BENGAL District-wise Total Workers Distribution")
