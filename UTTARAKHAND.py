...
#Data Load
UTTARAKHAND=pd.read_csv("D:/labour_project/UTTARAKHAND.csv",encoding="cp1252")
#Title
...
oly_total=UTTARAKHAND[UTTARAKHAND['NIC Name']=='Total']
...
with col1:
    st.write("STATE - UTTARAKHAND")
with col2:
    st.write("STATE CODE : 5")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(UTTARAKHAND)
...
sunburst_df = UTTARAKHAND[
    (UTTARAKHAND["Division"] != 0) &
    (UTTARAKHAND["Group"] != 0) &
    (UTTARAKHAND["Class"] != 0)]
...
coords = pd.read_csv("D:/labour_project/uts.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="UTTARAKHAND District-wise Total Workers Distribution")
