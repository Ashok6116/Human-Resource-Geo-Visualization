...
#Data Load
KERALA=pd.read_csv("D:/labour_project/KERALA.csv",encoding="cp1252")
#Title
...
oly_total=KERALA[KERALA['NIC Name']=='Total']
...
with col1:
    st.write("STATE - KERALA")
with col2:
    st.write("STATE CODE : 32")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(KERALA)
...
#   st.write(i)
#st.write(new_oly)
coords = pd.read_csv("D:/labour_project/keralam.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
