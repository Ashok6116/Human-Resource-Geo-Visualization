...
#Data Load
SIKKIM=pd.read_csv("D:/labour_project/SIKKIM.csv",encoding="cp1252")
#Title
...
oly_total=SIKKIM[SIKKIM['NIC Name']=='Total']
...
with col1:
    st.write("STATE - SIKKIM")
with col2:
    st.write("STATE CODE : 11")
def calculate_disCode(state):
    counts=state['District Code'].unique()
    return counts
dst_count=calculate_disCode(SIKKIM)
...
coords = pd.read_csv("D:/labour_project/sikkims.csv")
lat=list(coords['lat'])
log=list(coords['lon'])
...
fig = px.scatter_map(new_oly, lat="latitude", 
                  ...
                  color_continuous_scale=px.colors.cyclical.IceFire,
                    size_max=15, zoom=5, title="SIKKIM District-wise Total Workers Distribution")
